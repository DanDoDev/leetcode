...
const jumpEvaluator = (startingIndex, array, isGreaterThan) => {
    ...
    if(isGreaterThan){
        if(prevJumpsGreater[startingIndex] > -1) bestMoveIndex = prevJumpsGreater[startingIndex];
    }else{
        if(prevJumpsLess[startingIndex] > -1) bestMoveIndex = prevJumpsLess[startingIndex];
    }
   
    if(bestMoveIndex == -1){
        for(let i = startingIndex + 1;i < array.length;i++){
            const val = array[i];
            const legalMove = isGreaterThan && val >= currentVal || !isGreaterThan && val <=currentVal;
            const useMove = legalMove && (bestMoveIndex == -1 || (isGreaterThan && val < array[bestMoveIndex]) ||                           (!isGreaterThan && val > array[bestMoveIndex]));
